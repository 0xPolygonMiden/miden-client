...
impl SyncSummary {
    pub fn new(
        block_num: u32,
        new_notes: Vec<NoteId>,
        new_inclusion_proofs: Vec<NoteId>,
    ...
    pub fn is_empty(&self) -> bool {
        self.new_notes.is_empty()
            && self.new_inclusion_proofs.is_empty()
            && self.new_nullifiers.is_empty()
            && self.updated_onchain_accounts.is_empty()
    }
    ...
    pub fn combine_with(&mut self, mut other: Self) {
        ...
        self.new_inclusion_proofs.append(&mut other.new_inclusion_proofs);
        self.new_nullifiers.append(&mut other.new_nullifiers);
        self.updated_onchain_accounts.append(&mut other.updated_onchain_accounts);
    }
}
