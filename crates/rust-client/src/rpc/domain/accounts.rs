...
impl ProtoAccountStateHeader {
    /// Attempts to convert the response into a valid [StateHeaders].
    ///
    /// The response only contains the account's code if it was updated compared to what we sent
    /// as previously known (if any), so we need to provide it in case it was not sent.
    pub fn into_domain(
        self,
