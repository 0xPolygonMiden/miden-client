...
use super::{
  account_id::AccountId, note_id::NoteId, transaction_id::TransactionId
};
...
impl SyncSummary {
  ...
  pub fn committed_transactions(&self) -> Vec<TransactionId> {
    self.0.committed_transactions.iter().map(|transaction_id| transaction_id.into()).collect()
  }
}
