...
use super::{
    errors::ClientError, note_screener::{NoteRelevance, NoteScreener}, rpc::NodeRpcClient, store::{AuthInfo, Store, TransactionFilter}, Client
};
...
impl<N: NodeRpcClient, R: FeltRng, S: Store> Client<N, R, S> {
    ...
    pub async fn build_transaction_request(
        ...
        let account_auth = self.store.get_account_auth(account_id).await.unwrap();
    ...
    pub async fn new_transaction(
        ...
        let block_num = self.store.get_sync_height().await.unwrap();
    ...
    pub async fn submit_transaction(
        ...
        for (idx, note) in tx_result.created_notes().iter().enumerate() {
            let account_relevance = note_screener.check_relevance(note).await.unwrap();
        ...
        self.store.apply_transaction(tx_result).await.unwrap();
    ...
    async fn submit_proven_transaction_request(
        ...
        Ok(self.rpc_api.submit_proven_transaction(proven_transaction).await.unwrap())
    }
