...
use crate::native_code::errors::{ClientError, StoreError};
...
impl<S: Store> DataStore for ClientDataStore<S> {
    async fn get_transaction_inputs(
        ...
        for note_id in notes {
            let input_note_record = self.store.get_input_note(*note_id).await?;
...
async fn build_partial_mmr_with_paths<S: Store>(
    ...
    let authentication_paths =
        get_authentication_path_for_blocks(store, &block_nums, partial_mmr.forest()).await.unwrap();
...
pub async fn get_authentication_path_for_blocks<S: Store>(
    ...
    let mmr_nodes = store.get_chain_mmr_nodes(filter).await.unwrap();
