...
pub trait Store {
    ...
    async fn apply_transaction(&mut self, tx_result: TransactionResult) -> Result<(), StoreError>;
    ...
    async fn get_input_note(&self, note_id: NoteId) -> Result<InputNoteRecord, StoreError>;
    ...
    async fn insert_input_note(&mut self, note: &InputNoteRecord) -> Result<(), StoreError>;
    ...
    ///
    /// Returns a `StoreError::AccountDataNotFound` if there is no account for the provided ID
    async fn get_account_auth(&self, account_id: AccountId) -> Result<AuthInfo, StoreError>;
    ...
    async fn get_note_tags(&self) -> Result<Vec<u64>, StoreError>;

    /// Adds a note tag to the list of tags that the client is interested in.
    async fn add_note_tag(&mut self, tag: u64) -> Result<bool, StoreError>;
