...
impl<'a, S: Store> NoteScreener<'a, S> {
    ...
    pub async fn check_relevance(
        ...
        let account_ids = BTreeSet::from_iter(self.store.get_account_ids().await.unwrap());
        ...
        let note_relevance = match script_hash.as_str() {
            ...
            SWAP_NOTE_SCRIPT_ROOT => self.check_swap_relevance(note, &account_ids).await.unwrap(),
    ...
    async fn check_swap_relevance(
        ...
        for account_id in account_ids {
            let (account, _) = self.store.get_account(*account_id).await.unwrap();
...
//     use crate::native_code::note_screener::{
