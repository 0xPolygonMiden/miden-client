...
use super::{
    errors::{ClientError, StoreError}, rpc::{NodeRpcClient, CommittedNote, NoteDetails}, store::{ChainMmrNodeFilter, NoteFilter, Store, TransactionFilter}, transactions::TransactionRecord, Client // TODO: Add AuthInfo
};
...
impl<N: NodeRpcClient, R: FeltRng, S: Store> Client<N, R, S> {
    ...
    pub async fn get_note_tags(&self) -> Result<Vec<u64>, ClientError> {
        self.store.get_note_tags().await.map_err(|err| err.into())
    }

    /// Adds a note tag for the client to track.
    pub async fn add_note_tag(&mut self, tag: u64) -> Result<(), ClientError> {
    ...
    async fn sync_state_once(&mut self) -> Result<SyncStatus, ClientError> {
        ...
        let accounts: Vec<AccountStub> = self
            .store
            .get_account_stubs().await.unwrap()
            .into_iter()
    ...
    async fn get_note_details(
        ...
        let pending_input_notes =
            self.store.get_input_notes(NoteFilter::Pending).await?.into_iter().map(|n| n.id());

        let pending_output_notes =
            self.store.get_output_notes(NoteFilter::Pending).await?.into_iter().map(|n| n.id());
    ...
    async fn get_new_nullifiers(&self, new_nullifiers: Vec<Digest>) -> Result<Vec<Digest>, ClientError> {
