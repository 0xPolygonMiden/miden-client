...
The client will import all account-related data generated by the node stored as `*.mac` files, (located by default on the `./accounts` subdirectory of the node's root directory), and insert them in the local store. You should now be able to see the accounts by doing:
