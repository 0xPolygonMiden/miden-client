...
## Example: Transfer assets privately
...
The client will then import all account-related data generated by the node (stored as `*.mac` files), and insert them in the local store.  The accounts directory should be the one generated by the node when running the `make-genesis` command. 

Alternatively, you can run `miden-client account new` to create accounts as needed. For this example we are going to use two `basic-immutable` accounts, and one `fungible-faucet`. 

You can now list the imported accounts by running:
...
### 5. Transferring assets through private notes
