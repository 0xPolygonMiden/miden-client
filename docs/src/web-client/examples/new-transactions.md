...
```typescript
import { NoteType, TransactionProver } from "@demox-labs/miden-sdk";
...
To mint new tokens from a faucet:
...
try {
    const transactionRequest = webClient.newMintTransactionRequest(
        targetAccountId,  // Account receiving the tokens
        faucetId,         // Faucet account ID
        NoteType.Private, // Note type (Private or Public)
        1000             // Amount to mint
    );
