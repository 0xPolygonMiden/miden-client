...
impl Store {
    ...
    pub fn apply_state_sync(
        ...
        let previous_nodes = self.get_chain_mmr_nodes()?;
        ...
        if let Some(mmr_delta) = mmr_delta {
            ...
            let mut partial_mmr: PartialMmr = if current_block_num == 0 {
                ...
                let _leaves: Vec<Digest> = previous_nodes.values().cloned().collect();
                // TODO: this is probably incomplete but it should not matter for running a mint tx
                PartialMmr::from_peaks(current_peaks)
            };

            println!("partial mmr: {:?}", partial_mmr);
            println!("mmr delta: {:?}", mmr_delta);
            // apply the delta
            let new_authentication_nodes = partial_mmr
                .apply(mmr_delta.try_into().unwrap())
                .map_err(StoreError::MmrError)?;

            println!("new auth nodes: {:?}", new_authentication_nodes);
            println!("new partial mmr: {:?}", partial_mmr);
