...
fn list_accounts(client: Client) -> Result<(), String> {
    ...
    accounts.iter().for_each(|acc| {
        let account_type = match acc.id().account_type() {
            ...
            objects::accounts::AccountType::RegularAccountImmutableCode => {
                "Regular (Immutable code)"
            }
            objects::accounts::AccountType::RegularAccountUpdatableCode => "Regular",
        };
