...
use clap::Parser;
use miden_client::{config::Endpoint, store::sqlite_store::SqliteStore};
...
impl InitCmd {
    pub fn execute(&self, config_file_path: PathBuf) -> Result<(), String> {
        let mut cli_config = CliConfig::<SqliteStore>::default();
        if let Some(endpoint) = &self.rpc {
            let endpoint = Endpoint::try_from(endpoint.as_str()).map_err(|err| err.to_string())?;

            cli_config.client_config.rpc.endpoint = endpoint;
        }

        if let Some(path) = &self.store_path {
            cli_config.client_config.store.database_filepath = path.to_string();
        }
