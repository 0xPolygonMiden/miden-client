...
async fn test_swap_fully_onchain() {
    ...
    let client_1_consumable_notes =
        client_1.get_consumable_notes(Some(account_a.id())).await.unwrap();
    assert!(
    ...
    let tx_request = TransactionRequestBuilder::consume_notes(vec![account_a_mint_note_id])
        .build()
        .unwrap();
    execute_tx_and_sync(&mut client_1, account_a.id(), tx_request).await;

    // Sync and consume note for accountB
    client_2.sync_state().await.unwrap();
    let client_2_consumable_notes =
        client_2.get_consumable_notes(Some(account_b.id())).await.unwrap();
    assert!(
    ...
    let tx_request = TransactionRequestBuilder::consume_notes(vec![account_b_mint_note_id])
        .build()
        .unwrap();
    execute_tx_and_sync(&mut client_2, account_b.id(), tx_request).await;
    ...
    let tx = execute_tx_and_sync(&mut client_1, account_a.id(), tx_request).await;
    ...
    client_2.add_note_tag(swap_note_tag).await.unwrap();
    ...
    // At this point, the swap note should be the only note on client 2
    let swap_note_id = client_2
        ...
        .unwrap()
        .id();
    let tx_request = TransactionRequestBuilder::consume_notes(vec![swap_note_id]).build().unwrap();
    _ = execute_tx_and_sync(&mut client_2, account_b.id(), tx_request).await;
    ...
    // At this point, the payback note should be the only note on client 1
    let payback_note = &client_1.get_input_notes(NoteFilter::Unspent).await.unwrap()[0];
    client_1.sync_state().await.unwrap();
    println!("Consuming swap payback note on first client...");

    let tx_request = TransactionRequestBuilder::consume_notes(vec![payback_note.id()])
        .build()
        .unwrap();
    execute_tx_and_sync(&mut client_1, account_a.id(), tx_request).await;
    ...
    let account_a: Account = client_1.get_account(account_a.id()).await.unwrap().unwrap().into();
    ...
    let account_b: Account = client_2.get_account(account_b.id()).await.unwrap().unwrap().into();
...
async fn test_swap_private() {
    ...
    let output_note = client_1.get_output_note(output_notes[0].id()).await.unwrap().unwrap();
    ...
    .unwrap();
    client_2.add_note_tag(tag).await.unwrap();
    client_2
    ...
    client_1.sync_state().await.unwrap();
    println!("Consuming swap payback note on first client...");
    let payback_note_id = client_1.get_input_notes(NoteFilter::Committed).await.unwrap()[0].id();
    let tx_request =
        TransactionRequestBuilder::consume_notes(vec![payback_note_id]).build().unwrap();
    execute_tx_and_sync(&mut client_1, account_a.id(), tx_request).await;
