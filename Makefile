...
start-node: ## Run node. This requires the node repo to be present at `miden-node`
	cd $(NODE_DIR) && cargo run --bin miden-node $(NODE_FEATURES_TESTING) --locked -- bundled start --data-directory data --rpc.url http://localhost:57291  --block.interval 1250 --batch.interval 800
